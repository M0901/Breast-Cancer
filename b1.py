...
with container1:
    with col1:
        st.title('Relationship Between Measurements')
        scatter_fig
    with col2:
        st.title('Average Measurements Per Tumor Type')
        bar_fig
...
with container2:
    with col3:
        st.title('Distribution of Measurements')
        hist_fig
    with col4:
        st.title('Concentration of Measurements ')
        hexbin_fig
...
with col5:
    st.header("HeatMap")
    fig, ax = plt.subplots()
    sns.heatmap(breast_cancer_df.corr(), ax=ax)
    st.write(fig)
with col6:
    st.write('')    
...
#t = breast_cancer_df['target'].value_counts
